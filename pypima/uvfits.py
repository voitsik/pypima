...
class UVFits:
    ...
    def _read_header(self):
        ...
        for ind in range(2, 1 + header['NAXIS']):
            ...
            if ctype == 'FREQ':
                if naxis != 1:
                    raise UVFitsError('Multiple frequency setup not supported',
                                      self.file_name)
                self.freq = crval
            elif ctype == 'RA' or ctype == 'DEC':
                if naxis != 1:
                    raise UVFitsError('Multi source file not supported',
                                      self.file_name)
            elif ctype == 'STOKES':
                if naxis != 1:
                    raise UVFitsError('Multiple Stokes not supported',
                                      self.file_name)
    ...
    def _read_uv_data(self):
        ...
        self.dates = Time(self.dates1, self.dates2, scale='utc',
                          format='jd').datetime
        self.baselines = data.par('BASELINE')
        self.subarrays, self.ant1_inds, self.ant2_inds = \
            baseline_decode(self.baselines)
    ...
    def print_uv(self):
        ...
        for ind in range(self.gcount):
            ...
            for if_num in range(self.no_if):
                uu = self.u_raw[ind] * (self.freq +
                                        self.freq_table[if_num]['if_freq'])
                vv = self.v_raw[ind] * (self.freq +
                                        self.freq_table[if_num]['if_freq'])
                ...
                print(ind+1, if_num+1,
                      ant1_name, ant2_name, self.dates[ind],
    ...
    def useful_antennas(self):
        ...
        for ind in range(self.gcount):
            ant1_name, ant2_name = self.get_ant_by_ind(ind)
            for if_num in range(self.no_if):
                if self.amplitudes[ind, if_num] > 0 and \
                        self.weights[ind, if_num] > 0:
