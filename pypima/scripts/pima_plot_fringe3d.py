...
def plot(file_name, fri, format, title=False):
    ...
    if format == "png":
        ...
        dpi = None
    plt.savefig(file_name, format=format, bbox_inches="tight", pad_inches=0.1, dpi=dpi)
...
def main(args):
    ...
    if obs <= 0 or obs > pim.obs_number:
        print(
            "Incorrect observation number {} must be in range [{} {}]".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--title", action="store_true", help="add title to plot")
    parser.add_argument(
        "--format", default="pdf", help="plot file format (pdf, ps, png, ...)"
    )

    parser.add_argument(
        "-d", "--delay-window", type=float, help="delay window width in microseconds"
    )
    parser.add_argument("-r", "--rate-window", type=float, help="rate window width")
