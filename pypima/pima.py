...
class Pima:
    ...
    def fine(self, params: dict[str, Any] | None = None) -> str:
        ...
        log_name = os.path.join(
            self.work_dir, f"{self.exper}_{self.band}_{polar}_fine.log"
        )
        exc_obs_file = os.path.join(
            self.work_dir, f"{self.exper}_{self.band}_{polar}_fine_obs.exc"
        )
    ...
    def mk_time_flag_file(self, obs_ind: int, interval: float) -> None:
        """Create or update a time flag file.
        ...
        obs_ind : int
            Observation index.
        interval : float
            Time interval in seconds to flag the beginning or the end of the
            observation. If `interval` is positive flag the beginning of the
            observation, if negative - the end of the observation.

        """
        time_flag_file = os.path.join(
            self.work_dir, f"{self.exper}_{self.band}_time.flag"
        )
        tim_file = os.path.join(
            self.cnt_params["EXPER_DIR:"], self.cnt_params["SESS_CODE:"] + ".tim"
        )

        if not obs_ind and os.path.isfile(time_flag_file):
            os.remove(time_flag_file)
            self.update_cnt({"TIME_FLAG_FILE:": "NO"})
            return
        ...
        if not obs:
            self._error(f"mk_time_flag_file: observation {obs_ind} not found")

        # Read existing flags
        flags = defaultdict(list)
        if os.path.isfile(time_flag_file):
            with open(time_flag_file) as file:
                for line in file:
                    line = line.split("#")[0].strip()
                    ...
                    flags[int(cols[0])].append(int(cols[1]))

        if interval > 0:
            start_time = obs.start_time
            stop_time = start_time + timedelta(seconds=interval)
        else:
            stop_time = obs.stop_time
            start_time = stop_time + timedelta(seconds=interval)

        # Read tim file and find time indices to flag
        beg_time_ind = 0  # First time index of the observation
        with open(tim_file) as file:
            for line in file:
                ...
                time_ind = int(cols[1])
                time = datetime.strptime(cols[5][:26], "%Y.%m.%d-%H:%M:%S.%f")

                if beg_time_ind == 0 and time >= obs.start_time:
                    beg_time_ind = time_ind

                if beg_time_ind > 0 and start_time <= time <= stop_time:
                    flags[obs_ind].append(time_ind - beg_time_ind + 1)

                if time > stop_time:
                    break

        # Write flags to the file
        with open(time_flag_file, "w") as file:
            print("# PIMA_FLAG  Format of 2008.07.09", file=file)
            print("#", file=file)
            print(f"#  Created on {datetime.now()}", file=file)

            for obs_ind in sorted(flags):
                print("#", file=file)
                for time_ind in sorted(flags[obs_ind]):
                    print(f"{obs_ind:6d} {time_ind:6d}  0.00000", file=file)

            print("#", file=file)
            print("# PIMA_FLAG  Format of 2008.07.09", file=file)

        self.update_cnt({"TIME_FLAG_FILE:": time_flag_file})

    def fringe_plots(self) -> list[ObsFriPlots]:
        """Return list of plot files generated by ``frib`` task."""
        extentions = {
            "XW": "",
            "SAV": "sav",
            "GIF": "gif",
