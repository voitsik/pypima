...
ObsInfo = namedtuple("ObsInfo", ["id", "exper", "band", "sta1", "sta2", "start_time"])


def plot_theo_curves(ax_ampl, ax_snr, dur_arr, ampl_arr, snr_arr, max_coher_dur=400):
    """
    ...
    fit_params, _ = curve_fit(sqrt_func, dur_arr[coher_mask], snr_arr[coher_mask])
...
def save_txt(obs_info, dur_arr, ampl_arr, snr_arr):
    ...
    file_name = "{}_{}_{:02d}_coher.{}".format(
        obs_info.exper, obs_info.band, obs_info.id, "txt"
    )
...
def plot(obs_info, dur_arr, ampl_arr, snr_arr, out_format="pdf"):
    ...
    plot_file_name = "{}_{}_{:02d}_coher.{}".format(
        obs_info.exper, obs_info.band, obs_info.id, out_format
    )
    if out_format == "png":
        dpi = 300
    else:
...
def proc_obs(exper, band, obs, max_dur, plot_format="pdf"):
    ...
    obs_info = ObsInfo(obs, exper, band, fri[0]["sta1"], fri[0]["sta2"], start_time)
